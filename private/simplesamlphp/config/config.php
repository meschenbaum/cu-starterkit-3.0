...
$host = $_SERVER['HTTP_X_HOST'] ?? $_SERVER['HTTP_HOST'];
if (isset($_SERVER['HTTP_X_MASKED_PATH']) && $_SERVER['HTTP_X_MASKED_PATH']) {
    $saml_baseURL = 'https://'. $host . $_SERVER['HTTP_X_MASKED_PATH'];
} else {
    $saml_baseURL = 'https://'. $host;
}

$saml_baseurlpath = $saml_baseURL . '/simplesaml/';

if (isset($_REQUEST['HTTP_PANTHEON_ENVIRONMENT_DEBUG']) && $_REQUEST['HTTP_PANTHEON_ENVIRONMENT_DEBUG']) {
    print "<p><strong>SCRIPT_FILENAME:</strong> " . $_SERVER['SCRIPT_FILENAME'] . "</p>";
    print "<p><strong>Current Directory:</strong> " . __DIR__ . "</p>";
    print "<p><strong>Real Path:</strong> " . realpath('../../../../'). "</p>";
    if (file_exists('../../../../web/sites/default/settings.php')) {
      print "<p>Relative path to settings.php location is correct</p>";
    } else {
      print "<p>Relative path to settings.php location is NOT correct</p>";
    }
      print "<p><strong>Base URL:</strong> $saml_baseURL </p>";
      print "<p><strong>Base URL Path:</strong> $saml_baseurlpath </p>";
      print "<p><strong>Database:</strong> ";
      print_r($db);
      print "</p>";
      print "<p><strong>DSN:</strong> mysql:host=". $db['host'] .';port='. $db['port'] .';dbname='. $db['database']."</p>";
      print "<p>";
      ...
      switch ($_ENV['PANTHEON_ENVIRONMENT']) {
        case (str_starts_with($_ENV['PANTHEON_ENVIRONMENT'], 'ping-dev')):
            $idp_entityid = 'uisping_entity';
            break;
        case (str_starts_with($_ENV['PANTHEON_ENVIRONMENT'], 'ping-test')):
            $idp_entityid = 'uispingtst_entity';
            break;
        default:
            $idp_entityid = 'uisping_entity';
            break;
      }
      print "<p><strong>IdP EntityID:</strong> ".$idp_entityid."</p>";
      print "<h1>Pantheon Environment Debug</h1>";
      print "<h2>_ENV</h2>";
      print "<pre>";
      ksort($_ENV);
      print_r($_ENV);
...
$config = [
    ...
    'technicalcontact_email' => 's_webapps@cu.edu',
    ...
    */
     ...
     */
    'secretsalt' => '4vbeys6i016ityttgugiyb5yw0ilin1k',
     ...
     */
    'auth.adminpassword' => '$2y$10$dpcVD1CZFDWuIUPSoASuO.N4CdlyLwXfGSlnpih8MwXjrwRbZ51/W',
    ...
    'metadata.sign.algorithm' => null, //'http://www.w3.org/2001/04/xmldsig-more#rsa-sha256',
